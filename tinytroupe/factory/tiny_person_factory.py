...
class TinyPersonFactory(TinyFactory):
    ...
    def generate_person(
        ...
        logger.info(
            f"Starting the person generation based on that context: {self.context_text}"
        )
        ...
        prompt = chevron.render(
            open(self.person_prompt_template_path).read(),
            {
        ...
        def aux_generate(attempt):
            ...
            if message is not None:
                ...
                logger.debug(
                    f"At attempt {attempt}, generated person parameters:\n{json.dumps(result, indent=4, sort_keys=True)}"
                )
    ...
    @transactional
    def _aux_model_call(
        self, messages, temperature, frequency_penalty, presence_penalty
    ):
        """
        ...
        we don't want that.
        """
        return litellm_utils.client().send_message(
            messages,
            temperature=temperature,
            frequency_penalty=frequency_penalty,
            presence_penalty=presence_penalty,
