...
default["max_content_display_length"] = config["OpenAI"].getint(
    "MAX_CONTENT_DISPLAY_LENGTH", 1024
)
if config["OpenAI"].get("API_TYPE") == "azure":
    default["azure_embedding_model_api_version"] = config["OpenAI"].get(
...
if config["OpenAI"].get("API_TYPE") == "azure":
    ...
    llamaindex_openai_embed_model = OpenAIEmbedding(
        model=default["embedding_model"], embed_batch_size=10
    )
