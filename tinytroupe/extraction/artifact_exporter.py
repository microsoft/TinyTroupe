...
class ArtifactExporter(JsonSerializableRegistry):
    ...
    def export(
        ...
        if isinstance(artifact_data, str):
            ...
            raise ValueError(
                "The artifact data must be either a string or a dictionary."
            )
        ...
        for char in invalid_chars:
            ...
            if char in artifact_name:
                ...
                logger.warning(
                    f"Replaced invalid character {char} with hyphen in artifact name '{artifact_name}'."
                )

        artifact_file_path = self._compose_filepath(
            artifact_data, artifact_name, content_type, target_format, verbose
        )

        if target_format == "json":
            self._export_as_json(
                artifact_file_path, artifact_data, content_type, verbose
            )
        elif (
            target_format == "txt"
            or target_format == "text"
            or target_format == "md"
            or target_format == "markdown"
        ):
            self._export_as_txt(
                artifact_file_path, artifact_data, content_type, verbose
            )
        elif target_format == "docx":
            self._export_as_docx(
                artifact_file_path, artifact_data, content_format, verbose
            )
        else:
    ...
    def _export_as_json(
        ...
        with open(artifact_file_path, "w", encoding="utf-8") as f:
            if isinstance(artifact_data, dict):
                ...
                raise ValueError(
                    "The artifact data must be a dictionary to export to JSON."
                )
    ...
    def _export_as_docx(
        ...
        if content_original_format not in ["text", "txt", "markdown", "md"]:
            raise ValueError(
                f"The original format cannot be {content_original_format} to export to DOCX."
            )
        else:
            # normalize content value
            content_original_format = (
                "markdown"
                if content_original_format == "md"
                else content_original_format
            )
        ...
        pypandoc.convert_text(
            html_content, "docx", format="html", outputfile=artifact_file_path
        )
    ...
    def _compose_filepath(
        ...
        artifact_file_path = os.path.join(
            self.base_output_folder, subfolder, f"{artifact_name}.{extension}"
        )
