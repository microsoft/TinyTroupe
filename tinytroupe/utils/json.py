...
class JsonSerializableRegistry:
        ...
        for attr in serializable_attrs if serializable_attrs else self.__dict__:
            if attr not in suppress_attrs:
                ...
                if isinstance(value, JsonSerializableRegistry):
                    result[attr_renamed] = value.to_json(
                        serialization_type_field_name=serialization_type_field_name
                    )
    ...
    def from_json(
        ...
        for target_mro in target_class.__mro__:
            if hasattr(target_mro, "serializable_attributes") and isinstance(
                target_mro.serializable_attributes, list
            ):
                serializable_attrs.update(target_mro.serializable_attributes)
            if hasattr(target_mro, "custom_serialization_initializers") and isinstance(
                target_mro.custom_serialization_initializers, dict
            ):
                custom_serialization_initializers.update(
                    target_mro.custom_serialization_initializers
                )
            if hasattr(
                target_mro, "suppress_attributes_from_serialization"
            ) and isinstance(target_mro.suppress_attributes_from_serialization, list):
                ...
                if key in custom_serialization_initializers:
                    ...
                    for item in value:
                        if (
                            ...
                            and serialization_type_field_name in item
                        ):
    ...
    def _programmatic_name_to_json_name(cls, name):
        ...
        """
        if hasattr(cls, "serializable_attributes_renaming") and isinstance(
            cls.serializable_attributes_renaming, dict
        ):
        ...
        if hasattr(cls, "serializable_attributes_renaming") and isinstance(
            ...
            for k, v in cls.serializable_attributes_renaming.items():
                if v in reverse_rename:
                    raise ValueError(
                        f"Duplicate value '{v}' found in serializable_attributes_renaming."
                    )
                reverse_rename[v] = k
...
def merge_dicts(current, additions, overwrite=False, error_on_conflict=True):
    ...
    merged = (
        current.copy()
    )  # Create a copy of the current dictionary to avoid altering it
        ...
        if key in merged:
            ...
            # If both values are dictionaries, merge them recursively
                ...
                merged[key] = merge_dicts(
                    merged[key], additions[key], overwrite, error_on_conflict
                )
            ...
            # If the values are of different types, raise an exception
                ...
                raise TypeError(
                    f"Cannot merge different types: {type(merged[key])} and {type(additions[key])} for key '{key}'"
                )
            # If the values are of the same type but not both lists/dictionaries, decide based on the overwrite parameter
                ...
                if overwrite:
                    ...
                    if error_on_conflict:
                        raise ValueError(
                            f"Conflict at key '{key}': overwrite is set to False and values are different."
                        )
                    else:
...
def extract_json(text: str) -> dict:
    ...
    # Helper function to clean and load JSON
    def _clean_and_load_json(json_string: str) -> dict:
    ...
    for i, char in enumerate(text):
        if char == "{":
            if not stack:  # Start of a new potential JSON object
                start_index = i
            stack.append(char)
        elif char == "}":
    ...
    for candidate in string_candidates:
        try:
            ...
            logger.debug(
                f"Unexpected error during JSON decode from candidate: {e}. Skipping."
            )
            continue
    ...
    best_priority = (
        -1
    )  # 3: action+cognitive_state, 2: action, 1: cognitive_state, 0: any dict
