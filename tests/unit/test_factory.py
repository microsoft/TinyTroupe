...
def test_generate_people(setup):
    ...
    people = factory.generate_people(
        10,
        agent_particularities="A random person from the American population.",
        verbose=True,
    )
    ...
    for person in people:
        ...
        nationality_acceptable = nationality is not None and (
            "american" in nationality.lower()
            or "united states" in nationality.lower()
            or "u.s." in nationality.lower()
            or "usa" in nationality.lower()
        )
        assert (
            nationality_acceptable
        ), f"Expected American nationality variant, got: {nationality}"
        assert person.get("age") is not None and person.get("age") > 0
...
def test_generate_people_2(setup):
    ...
    people = factory.generate_people(
        10,
        agent_particularities="A random person from the American population with American nationality.",
        verbose=True,
    )
...
def test_generate_people_with_different_particularities(setup):
    ...
    professionals = factory.generate_people(
        5, agent_particularities="Working professionals in various fields", verbose=True
    )

    # Generate students
    students = factory.generate_people(
        3,
        agent_particularities="College students from diverse backgrounds",
        verbose=True,
    )
...
def test_create_factory_from_demography_dict(setup):
    ...
    demography_data = {
        "country": "Canada",
        "age_distribution": {"20-35": 0.4, "36-55": 0.4, "56+": 0.2},
...
def test_factory_with_sampling_plan_initialization(setup):
    ...
    people = factory.generate_people(
        5, agent_particularities="Working professionals", verbose=True
    )
    ...
    more_people = factory.generate_people(
        3, agent_particularities="Senior executives", verbose=True
    )
...
def test_factory_name_uniqueness_across_factories(setup):
    ...
    people2 = factory2.generate_people(
        3, agent_particularities="Financial analysts and advisors."
    )
...
def test_factory_error_handling(setup):
    ...
    with pytest.raises(
        ValueError, match="Cannot generate more people than the population size"
    ):
...
def test_factory_complex_market_research_scenario(setup):
    ...
    singles = singles_factory.generate_people(
        5, agent_particularities="Single adults interested in travel"
    )
    families = families_factory.generate_people(
        5, agent_particularities="Parents planning family vacations"
    )
    couples = couples_factory.generate_people(
        5, agent_particularities="Couples seeking romantic getaways"
    )
...
def test_large_scale_generation_multiple_industries(setup):
    ...
    tech_people = tech_factory.generate_people(
        100,
        agent_particularities="Technology professionals with diverse skills and experience levels",
    )
    healthcare_people = healthcare_factory.generate_people(
        100, agent_particularities="Healthcare workers from various medical specialties"
    )
    finance_people = finance_factory.generate_people(
        100,
        agent_particularities="Financial professionals with different roles and expertise",
    )
    education_people = education_factory.generate_people(
        100,
        agent_particularities="Education professionals from various levels and subjects",
    )

    # Verify counts are in expected range (100 ± 10)
    assert (
        90 <= len(tech_people) <= 110
    ), f"Tech people count {len(tech_people)} outside expected range"
    assert (
        90 <= len(healthcare_people) <= 110
    ), f"Healthcare people count {len(healthcare_people)} outside expected range"
    assert (
        90 <= len(finance_people) <= 110
    ), f"Finance people count {len(finance_people)} outside expected range"
    assert (
        90 <= len(education_people) <= 110
    ), f"Education people count {len(education_people)} outside expected range"
    ...
    assert len(unique_names) == len(
        all_names
    ), f"Found {len(all_names) - len(unique_names)} duplicate names across factories"

    # Verify total count
    total_generated = len(all_people)
    assert (
        360 <= total_generated <= 440
...
def test_large_scale_generation_geographic_regions(setup):
    ...
    usa_people = usa_factory.generate_people(
        100, agent_particularities="Americans who enjoy travelling by train."
    )
    europe_people = europe_factory.generate_people(
        100, agent_particularities="Europeans who enjoy travelling by train."
    )
    asia_people = asia_factory.generate_people(
        100, agent_particularities="Asians who enjoy travelling by train."
    )

    # Verify counts are in expected range
    assert (
        90 <= len(usa_people) <= 110
    ), f"USA people count {len(usa_people)} outside expected range"
    assert (
        90 <= len(europe_people) <= 110
    ), f"Europe people count {len(europe_people)} outside expected range"
    assert (
        90 <= len(asia_people) <= 110
    ), f"Asia people count {len(asia_people)} outside expected range"
    ...
    for person in all_people:
        ...
        assert (
            len(bio) > 50
        ), f"Person {person.name} has insufficient biography length: {len(bio)}"
    ...
    europe_matches = sum(
        1 for bio in europe_bios if any(term in bio for term in europe_terms)
    )
    asia_matches = sum(
        1 for bio in asia_bios if any(term in bio for term in asia_terms)
    )

    # At least some people should have region-specific indicators
    assert (
        usa_matches >= len(usa_people) * 0.3
    ), f"Too few USA people with regional indicators: {usa_matches}/{len(usa_people)}"
    assert (
        europe_matches >= len(europe_people) * 0.3
    ), f"Too few Europe people with regional indicators: {europe_matches}/{len(europe_people)}"
    assert (
        asia_matches >= len(asia_people) * 0.3
    ), f"Too few Asia people with regional indicators: {asia_matches}/{len(asia_people)}"
...
def test_large_scale_generation_performance_and_consistency(setup):
    ...
    people = factory.generate_people(
        100, agent_particularities="Urban professionals with diverse career backgrounds"
    )

    # Verify count is in expected range (even wider range to account for LLM variability and conservative generation)
    assert (
        50 <= len(people) <= 120
    ), f"Generated {len(people)} people, expected 50-120 (allowing for LLM variability and conservative generation)"
    ...
    for person in people:
        ...
        assert (
            len(bio) > 30
        ), f"Person {person.name} has insufficient biography: {len(bio)}"
        bios.append(bio)
    ...
    if ages:
        assert (
            len(set(ages)) > 20
        ), f"Age diversity too low: {len(set(ages))} unique ages"
    ...
    additional_people = factory.generate_people(
        15, agent_particularities="Additional urban professionals"
    )
    assert (
        10 <= len(additional_people) <= 25
    ), f"Additional batch size {len(additional_people)} outside expected range 10-25 (allowing for LLM variability)"

    # Verify total doesn't exceed population size
    total_generated = len(people) + len(additional_people)
    assert (
        total_generated <= 120
    ), f"Total generated {total_generated} exceeds population size of 120"

    # Verify no name conflicts with previous batch
    all_names = names + [p.name for p in additional_people]
    assert len(set(all_names)) == len(
        all_names
    ), "Names should remain unique across batches"
