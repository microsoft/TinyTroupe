...
def test_memory_operations(setup):
    ...
    for agent in [create_oscar_the_architect(), create_lisa_the_data_scientist()]:
        ...
        assert (
            len(relevant_memories) >= 0
        ), f"{agent.name} should be able to retrieve relevant memories"
        ...
        ), f"{agent.name} should have fewer memories after clearing"
...
def test_mental_faculties(setup):
    ...
    assert (
        custom_faculty in agent._mental_faculties
    ), f"{agent.name} should have the custom faculty"
    ...
    assert (
        tool_faculty in agent._mental_faculties
    ), f"{agent.name} should have the tool faculty"
...
def test_agent_accessibility_management(setup):
    ...
    assert (
        lisa in oscar._accessible_agents
    ), f"{oscar.name} should have {lisa.name} as accessible"

    # Test making multiple agents accessible
    oscar.make_agents_accessible([marcos], relation_description="Team member")
    assert (
        marcos in oscar._accessible_agents
    ), f"{oscar.name} should have {marcos.name} as accessible"

    # Test making agent inaccessible
    oscar.make_agent_inaccessible(lisa)
    assert (
        lisa not in oscar._accessible_agents
    ), f"{oscar.name} should not have {lisa.name} as accessible after removal"

    # Test making all agents inaccessible
    oscar.make_all_agents_inaccessible()
    assert (
        len(oscar._accessible_agents) == 0
    ), f"{oscar.name} should have no accessible agents"
...
def test_action_generation_and_flow(setup):
    ...
    assert contains_action_type(
        actions_think, "THINK"
    ), f"{agent.name} should think when seeing something interesting"
...
def test_relationship_management(setup):
    ...
    relationship_names = [
        r.get("Name") for r in oscar._persona.get("relationships", [])
    ]
    assert (
        lisa.name in relationship_names
    ), f"{oscar.name} should have relationship with {lisa.name}"
    assert (
        marcos.name in relationship_names
    ), f"{oscar.name} should have relationship with {marcos.name}"
    ...
    assert (
        len(relationships_after_clear) == 0
    ), f"{oscar.name} should have no relationships after clearing"
...
def test_document_reading_capabilities(setup):
    ...
    try:
        ...
        assert (
            len(agent.semantic_memory.documents) > 0
        ), f"{agent.name} should have documents in semantic memory"
...
def test_web_document_reading(setup):
    ...
    try:
        ...
        web_read_memories = [
            m for m in memories if "WEB_DOCUMENT" in str(m.get("content", {}))
        ]
        assert (
            len(web_read_memories) > 0
        ), "Agent should have memories of reading web document"
    ...
    web_urls = ["https://httpbin.org/json", "https://httpbin.org/uuid"]
    ...
    try:
        ...
        assert len(web_read_memories) >= len(
            web_urls
        ), "Agent should have memories for each web document"
...
def test_cognitive_state_management(setup):
    ...
    assert (
        "context" in initial_state
    ), f"{agent.name} should have context in mental state"
            ...
            if isinstance(content, dict) and "stimuli" in content:
                for stimulus in content["stimuli"]:
                    if (
                        ...
                        and stimulus.get("type") == "INTERNAL_GOAL_FORMULATION"
                    ):
                        stimulus_content = stimulus.get("content", "")
                        if (
                            "sustainable" in stimulus_content.lower()
                            or "building" in stimulus_content.lower()
                        ):
                            goal_found = True
                            break
    ...
    assert (
        goal_found
    ), f"{agent.name} should have internalized the building goal in memory"
    ...
    assert (
        "creative" in current_context
    ), f"{agent.name} should have creative in context"

    # Test location changes
    agent.move_to("New York", context=["urban", "busy"])
    assert (
        agent._mental_state["location"] == "New York"
    ), f"{agent.name} should be in New York"
    assert (
        "urban" in agent._mental_state["context"]
    ), f"{agent.name} should have urban context"
...
def test_memory_retrieval_strategies(setup):
    ...
    assert (
        len(relevant_memories) >= 0
    ), f"{agent.name} should retrieve context-relevant memories"

    # Test memory summarization
    summary = agent.summarize_relevant_memories_via_full_scan("patient treatment")
    assert isinstance(
        summary, str
    ), f"{agent.name} should generate memory summary as string"
    ...
    if (
        summary and len(summary.strip()) > 10
    ):  # Only check if we have substantial content
        assert proposition_holds(
            summary
            + " - The summary relates to medical topics, patient care, or healthcare"
        )

    # Test retrieving specific memory ranges
    memories_range = agent.retrieve_memories(first_n=2, last_n=2)
    assert (
        len(memories_range) >= 2
    ), f"{agent.name} should retrieve memories in specified range"
...
def test_persona_manipulation_edge_cases(setup):
    ...
    assert (
        "Python" in agent._persona["skills"]
    ), f"{agent.name} should have Python skill"
    ...
    assert (
        "Python" in all_skills and "R" in all_skills
    ), f"{agent.name} should have merged skills"

    # Test including additional persona definitions
    additional_defs = {"certifications": ["AWS", "Azure"], "experience_years": 5}
    agent.include_persona_definitions(additional_defs)
    assert (
        "certifications" in agent._persona
    ), f"{agent.name} should have certifications"
    assert (
        agent._persona["experience_years"] == 5
    ), f"{agent.name} should have experience years"
    ...
    try:
        ...
        assert (
            "personality_traits" in agent._persona
        ), f"{agent.name} should have imported personality traits"
...
def test_serialization_edge_cases(setup):
    ...
    assert (
        "_mental_state" in complete_state
    ), "Complete state should include _mental_state"
    ...
    assert (
        decoded_agent._persona["projects"] == agent._persona["projects"]
    ), "Decoded agent should have same projects"
    ...
    assert (
        loaded_agent.name == f"{agent.name}_loaded"
    ), "Loaded agent should have new name"
...
def test_communication_and_display(setup):
    ...
    assert (
        len(agent._displayed_communications_buffer) > 0
    ), f"{agent.name} should have communications in buffer"

    # Test clearing communications buffer
    agent.clear_communications_buffer()
    assert (
        len(agent._displayed_communications_buffer) == 0
    ), f"{agent.name} should have empty communications buffer"

    # Test pretty printing functionality
    pretty_output = agent.pretty_current_interactions(simplified=True)
    assert isinstance(
        pretty_output, str
    ), f"{agent.name} should generate string output for pretty interactions"

    # Test minibio generation
    bio = agent.minibio(extended=True)
    assert (
        isinstance(bio, str) and len(bio) > 0
    ), f"{agent.name} should generate non-empty bio"

    # Semantic verification: check that the bio is actually about the agent's persona
    agent_name = agent.name.split()[0]  # First name
    assert proposition_holds(
        f"The following text is a biographical description of a person named {agent_name}: '{bio}'"
    ...
    assert timestamp is None or (
        isinstance(timestamp, str) and len(timestamp) > 0
    ), f"{agent.name} should generate valid timestamp or None"
...
def test_action_buffer_management(setup):
    ...
    assert (
        len(actions_after_pop) == 0
    ), f"{agent.name} should have empty actions buffer after popping"
    ...
    assert isinstance(
        talk_content, str
    ), f"{agent.name} should return TALK content as string"
    ...
    all_talk_content = agent.pop_actions_and_get_contents_for(
        "TALK", only_last_action=False
    )
    assert isinstance(
        all_talk_content, str
    ), f"{agent.name} should return all TALK content as string"
...
def test_environment_interaction(setup):
    ...
    assert (
        lisa in oscar._accessible_agents
    ), f"{oscar.name} should have access to {lisa.name} in same world"
    assert (
        oscar in lisa._accessible_agents
    ), f"{lisa.name} should have access to {oscar.name} in same world"
    ...
    broadcast_received_oscar = any(
        "Welcome to the office" in str(memory) for memory in oscar_memories
    )
    broadcast_received_lisa = any(
        "Welcome to the office" in str(memory) for memory in lisa_memories
    )

    assert (
        broadcast_received_oscar or broadcast_received_lisa
    ), "At least one agent should have received the broadcast"
    ...
    assert len(oscar_post_run_memories) >= len(
        oscar_memories
    ), f"{oscar.name} should have more memories after simulation"
    assert len(lisa_post_run_memories) >= len(
        lisa_memories
    ), f"{lisa.name} should have more memories after simulation"
...
def test_error_handling_robustness(setup):
    ...
    try:
        ...
        ), "Should raise appropriate exception for None memory"
    ...
    try:
        ...
        assert isinstance(
            memories, list
        ), "Should return list even with invalid parameters"
    except Exception:
...
def test_configuration_management(setup):
    ...
    assert (
        agent._persona["occupation"] == "Senior Data Scientist"
    ), "Should update occupation"
    ...
    assert (
        "PMP" in agent._persona["certifications"]
    ), "Should contain specific certification"

    # Test behavior consistency after persona changes
    actions = agent.listen_and_act(
        "Tell me about your qualifications", return_actions=True
    )
    ...
    if talk_actions:
        ...
        assert (
            "Senior" in talk_content
            or "Data Scientist" in talk_content
            or "certification" in talk_content.lower()
        ), "Should reflect updated persona in responses"

    # Test mental state configuration
    agent.change_context(["laboratory", "research", "focused"])
    assert (
        "laboratory" in agent._mental_state["context"]
    ), "Should update mental context"
    assert (
        "research" in agent._mental_state["context"]
    ...
    assert (
        agent._persona["occupation"] == "Senior Data Scientist"
    ), "Occupation should persist across actions"
    assert (
        marcos in agent._accessible_agents
    ), "Agent accessibility should persist across actions"
...
def test_performance_and_limits(setup):
    ...
    all_memories = agent.episodic_memory.retrieve_all()
    assert (
        len(all_memories) >= large_memory_count
    ), f"Should store at least {large_memory_count} memories"
    ...
    assert (
        retrieval_time < 5.0
    ), "Memory retrieval should be reasonably fast"  # Reasonable timeout

    # Test action generation limits
    start_time = time.time()
    try:
        actions = agent.listen_and_act(
            "Please provide a detailed analysis of all your recent cases",
            return_actions=True,
        )
        ...
        assert (
            len(actions) <= 20
        ), "Should respect reasonable action limits"  # Prevent runaway generation
        assert (
            action_generation_time < 60.0
        ), "Action generation should complete in reasonable time (60s for gpt-5-mini)"
    except Exception as e:
        ...
        assert (
            action_generation_time < 60.0
        ), "Should fail fast if there are issues (within 60s)"
...
def test_integration_scenarios(setup):
    ...
    world.broadcast(
        "We're starting a new project: designing a smart hospital with AI-powered patient monitoring."
    )

    # Each agent contributes their expertise
    architect.internalize_goal(
        "Design hospital architecture optimized for technology integration"
    )
    ...
    has_collaboration = any(
        indicator in combined_content for indicator in collaboration_indicators
    )
    has_project_focus = any(
        indicator in combined_content for indicator in project_indicators
    )
    ...
    try:
        post_collaboration_actions = architect.listen_and_act(
            "Summarize the key architectural requirements from our discussion",
            return_actions=True,
        )
        if post_collaboration_actions is not None:
            assert (
                len(post_collaboration_actions) > 0
            ), "Agent should be able to continue working after collaboration"
        else:
            logger.warning(
                "Agent returned None for post-collaboration actions - this may indicate system state issues"
            )
    except Exception as e:
...
def test_advanced_memory_retrieval(setup):
    ...
    if len(summary) > 0:
        ...
        assert any(
            ...
            for keyword in ["architecture", "sustainability", "design", "building"]
...
def test_fragment_import_system(setup):
    ...
    fragment_content = {
        "type": "Fragment",
        "persona": {
            "political_views": {"economic": "moderate", "social": "progressive"},
    ...
    try:
        ...
        assert (
            agent.get("political_views") is not None
        ), "Political views should be imported"
        assert agent.get("hobbies") is not None, "Hobbies should be imported"
        assert "moderate" in str(
            agent.get("political_views")
        ), "Economic views should be imported"
...
def test_complex_context_management(setup):
    ...
    agent.move_to(
        "Client's office",
        context=["Meeting with stakeholders", "Presenting design options"],
    )
...
def test_multi_faculty_integration(setup):
    ...
    agent.listen_and_act(
        "Create a detailed architectural proposal document and analyze the building costs"
    )
