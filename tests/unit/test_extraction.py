...
def test_export_json(exporter):
    ...
    exporter.export(
        "test_artifact", artifact_data, content_type="record", target_format="json"
    )

    # check if the JSON file was exported correctly
    assert os.path.exists(
        f"{EXPORT_BASE_FOLDER}/record/test_artifact.json"
    ), "The JSON file should have been exported."

    # does it contain the data?
    with open(f"{EXPORT_BASE_FOLDER}/record/test_artifact.json", "r") as f:
        exported_data = json.load(f)
        assert (
            exported_data == artifact_data
        ), "The exported JSON data should match the original data."
...
def test_export_text(exporter):
    ...
    exporter.export(
        "test_artifact", artifact_data, content_type="text", target_format="txt"
    )

    # check if the text file was exported correctly
    assert os.path.exists(
        f"{EXPORT_BASE_FOLDER}/text/test_artifact.txt"
    ), "The text file should have been exported."
    ...
    with open(f"{EXPORT_BASE_FOLDER}/text/test_artifact.txt", "r") as f:
        ...
        assert (
            exported_data == artifact_data
        ), "The exported text data should match the original data."
...
def test_export_docx(exporter):
    ...
    assert os.path.exists(
        f"{EXPORT_BASE_FOLDER}/Document/test_artifact.docx"
    ...
    assert (
        "#" not in exported_data
    ), "The exported docx data should not contain Markdown."
...
def test_normalizer():
    ...
    assert (
        len(normalizer.normalized_elements) == 10
    ), "The number of normalized elements should be equal to the specified value."

    # sample 5 random elements from concepts using standard python methods

    random_concepts_buckets = [
    ...
    assert (
        len(normalizer.normalizing_map.keys()) == 0
    ), "The normalizing map should be empty at the beginning."
    for bucket in random_concepts_buckets:
        ...
        ), "The normalized concept should not be None."
        ...
        # check same length
        assert len(normalized_concept) == len(
            bucket
        ...
        for element in bucket:
            assert (
                element in normalizer.normalizing_map.keys()
            ), f"{element} should be in the normalizing map keys."

        assert (
            next_cache_size > 0
        ), "The cache size should be greater than 0 after normalizing a new concept."
        assert (
            next_cache_size >= init_cache_size
        ), "The cache size should not decrease after normalizing a new concept."
