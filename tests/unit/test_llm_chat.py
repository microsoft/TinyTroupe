...
class TestLLMChat:
    ...
    def test_initialization(self):
        ...
        chat = LLMChat(
            system_prompt="You are a helpful assistant.", user_prompt="Tell me a joke."
        )
    ...
    def test_initialization_validation_errors(self):
        ...
        with pytest.raises(
            ValueError, match="Either the template or the prompt must be specified"
        ):
            LLMChat(system_template_name="test.mustache", system_prompt="Test prompt")
    ...
    def test_simple_call(self):
        ...
        assert (
            len(chat.conversation_history) > 0
        ...
        assert proposition_holds(
            f"The following text is a joke about programming or coding: '{response}'"
    ...
    def test_add_messages(self):
        ...
        chat = LLMChat(
            system_prompt="You are a helpful assistant.", user_prompt="Initial message."
        )
    ...
    def test_dict_output(self):
        ...
        assert any(
            key in response_str for key in ["title", "author", "book"]
    ...
    def test_list_output(self):
        ...
        assert proposition_holds(
            f"The following list contains names of countries: '{response_str}'"
    ...
    def test_coerce_to_enumerable_valid_inputs(self):
        ...
        assert (
            chat._coerce_to_enumerable("I choose green", options) == "green"
        )  # embedded
    ...
    def test_call_with_mock_response(self, mock_client):
        ...
        chat = LLMChat(system_prompt="Test system", user_prompt="Test user")
    ...
    def test_add_user_message_validation(self):
        ...
        with pytest.raises(
            ValueError, match="Either message or template_name must be specified"
        ):
    ...
    def test_method_chaining(self):
        ...
        result = chat.add_user_message("Test message").add_system_message(
            "System message"
        )
    ...
    def test_request_message_methods(self):
        ...
        assert (
            "number" in float_msg["content"].lower()
            or "float" in float_msg["content"].lower()
        )
...
class TestLLMPydanticModels:
    ...
    def test_llm_scalar_with_justification_response(self):
        ...
        response = LLMScalarWithJustificationResponse(
            justification="This is my reasoning", value="test_value", confidence=0.85
        )
    ...
    def test_pydantic_model_validation(self):
        ...
        with pytest.raises(Exception):  # Pydantic validation error
            LLMScalarWithJustificationResponse(
                justification="Test", value="test", confidence="not_a_number"
            )
