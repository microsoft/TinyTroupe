...
class BadActionInjector:
    ...
    def add_bad_action(self, action_type, content, cognitive_state=None):
        ...
        bad_action = {"type": action_type, "content": content}
        bad_content = {
            "action": bad_action,
    ...
    def custom_violation(
        self, action_type, content, goals=None, attention=None, emotions=None
    ):
        """Generic method to inject any custom violation."""
        cognitive_state = {
            "goals": goals
            or f"Pursuing misguided objectives related to: {content[:50]}...",
    ...
    def create_patched_method(self):
        ...
        def patched_generate_tentative_action(
            agent,
            current_messages,
            ...
            if (
                feedback_from_previous_attempt is None
                and injector.current_bad_action_index < len(injector.bad_actions)
            ):

                bad_action, role, content = injector.bad_actions[
                    injector.current_bad_action_index
                ]
                injector.current_bad_action_index += 1

                logger.info(
                    f"[BAD ACTION] Injecting bad action #{injector.current_bad_action_index}: {bad_action.get('content', '')[:100]}..."
                )
                return bad_action, role, content
            else:
                # Use real API for correction attempts or when no more bad actions
                logger.info(
                    f"[REAL API] Using real API for correction attempt (call #{injector.call_count})"
                )
                return injector.original_method(
                    agent,
                    current_messages,
                    feedback_from_previous_attempt,
                    previous_tentative_action,
                    previous_llm_role,
                    previous_llm_content,
                )
    ...
    def get_stats(self):
        ...
        return {
            ...
            "bad_actions_remaining": len(self.bad_actions)
            - self.current_bad_action_index,
        }
...
def test_action_generator_initialization():
    ...
    custom_generator = ActionGenerator(
        max_attempts=5, enable_quality_checks=False, quality_threshold=5
    )
...
def test_action_generator_with_agent(setup):
    ...
    assert terminates_with_action_type(
        actions, "DONE"
    ), "Should terminate with DONE action"

    # Semantic verification: ensure the response is actually about Oscar (an architect)
    action_content = safe_get_action_content(
        actions, -2
    )  # Get the main response action (before DONE)
    if action_content and len(action_content) > 20:  # Only check substantial responses
        assert proposition_holds(
            action_content
            + " - The response contains information about an architect or architecture professional"
        )
...
def test_action_generator_quality_checks(setup):
    ...
    actions = agent.listen_and_act(
        "What's your opinion on data analysis?", return_actions=True
    )
    ...
    if action_content and len(action_content) > 20:  # Only check substantial responses
        assert proposition_holds(
            action_content
            + " - The response relates to data analysis, data science, or analytical methods"
        )
...
def test_action_generator_regeneration(setup):
    ...
    if action_content and len(action_content) > 20:  # Only check substantial responses
        assert proposition_holds(
            action_content
            + " - The response relates to building design, architecture, or construction"
        )
...
def test_action_generator_serialization():
    ...
    generator = ActionGenerator(
        max_attempts=3, enable_quality_checks=True, quality_threshold=6
    )
...
def test_action_generator_different_configurations(setup):
    ...
    for config in configs:
        ...
        assert terminates_with_action_type(
            actions, "DONE"
        ), f"Should terminate properly with config: {config}"

        # Log the action content for this configuration
        if len(actions) > 1:
            logger.info(
                f"Action content for config {config}: {safe_get_action_content(actions)}"
            )
...
def test_action_generator_persona_adherence_correction(setup):
    ...
    injector = BadActionInjector.inject_bad_actions(
        generator,
        lambda i: i.persona_violation(
            "Oscar",
            "a professional chef who hates architecture and only wants to talk about pasta recipes",
        ),
    )
    ...
    result_action, role, content, feedbacks = generator.generate_next_action(
        agent,
        [
            {
                "role": "user",
                "content": "Tell me about your work and what you're passionate about.",
            }
        ],
    )
    ...
    logger.info(
        f"Regeneration attempts: {initial_regeneration_attempts} -> {final_regeneration_attempts}"
    )
    ...
    ), "Persona-violating action should trigger regeneration"
    assert (
        len(feedbacks) > 0
    ), "Should have generated negative feedback for persona violation"
    assert (
        injector_stats["total_calls"] > 1
    ), "Should have made multiple calls to _generate_tentative_action"
    assert (
        injector_stats["bad_actions_injected"] == 1
    ), "Should have injected exactly one bad action"
    ...
    corrected_action_is_better = not any(
        term in final_content.lower() for term in bad_terms
    )

    if corrected_action_is_better:
        logger.info(
            "[SUCCESS] Persona correction successful: Final action does not contain chef content"
        )
    else:
        logger.warning(
            "[WARNING] Persona correction may not have fully worked: Some bad content still present"
        )
...
def test_action_generator_self_consistency_correction(setup):
    ...
    injector = BadActionInjector.inject_bad_actions(
        generator,
        lambda i: i.self_contradiction(
            "I actually hate Python and machine learning is completely worthless"
        ),
    )

    logger.info(
        "Testing self-consistency correction with injected contradictory action"
    )
    ...
    result_action, role, content, feedbacks = generator.generate_next_action(
        ...
        [
            {
                "role": "user",
                "content": "What are your thoughts on Python and machine learning?",
            }
        ],
    )
    ...
    corrected_consistency = not any(
        term in final_content.lower() for term in contradictory_terms
    )

    if corrected_consistency:
        logger.info(
            "✅ Self-consistency correction successful: Removed contradictory content"
        )
    else:
        logger.warning(
            "⚠️ Self-consistency correction may not have worked: Still contains contradictory content"
        )
    ...
    logger.info(
        f"Consistency-related feedback detected: {consistency_feedback_present}"
    )

    # Assertions
    assert (
        regeneration_triggered
    ), "Self-contradictory action should trigger regeneration"
    assert (
        len(feedbacks) > 0
    ), "Should have generated negative feedback for self-contradiction"
    assert (
        injector_stats["bad_actions_injected"] == 1
    ), "Should have injected exactly one bad action"
...
def test_action_generator_direct_correction_mechanism(setup):
    ...
    for msg in challenging_messages:
        ...
        if len(actions) > 1:
            ...
            logger.info(
                f"Final action after potential direct correction: {final_content}"
            )

    stats = generator.get_statistics()
    logger.info(f"Direct correction test statistics: {stats}")

    direct_corrections_triggered = (
        generator.direct_correction_attempts - initial_direct_correction_attempts
    )
    logger.info(f"Direct corrections triggered: {direct_corrections_triggered}")

    assert (
        direct_corrections_triggered >= 0
    ), "Should handle direct correction mechanism"


def _create_comprehensive_generator(*, max_action_similarity=0.6):
    """Helper to create a generator with all quality checks enabled.
    
    Args:
        max_action_similarity: Maximum allowed similarity threshold (0.0-1.0). 
                               Default 0.6 is reasonable for most tests; use 0.4 
                               for strict similarity testing.
    """
    return ActionGenerator(
        ...
        quality_threshold=6,
        max_action_similarity=max_action_similarity,
        continue_on_failure=True,
    )
...
def test_comprehensive_stress_1_persona_violation(setup):
    ...
    generator = _create_comprehensive_generator()
    agent = create_unique_oscar("_stress_persona")
    agent.action_generator = generator
    ...
    injector = BadActionInjector.inject_bad_actions(
        ...
        lambda i: i.persona_violation(
            "Oscar",
            "a professional chef who hates architecture and only talks about pasta",
        ),
    )

    initial_regeneration = generator.regeneration_attempts
    result, _, _, feedbacks = generator.generate_next_action(
        agent, [{"role": "user", "content": "Tell me about your work."}]
    )
    ...
    # Assertions
    assert result is not None, "System should handle persona test without crashing"
    injector_stats = injector.get_stats()
    logger.info(f"Injector stats: {injector_stats}")
...
def test_comprehensive_stress_2_self_consistency_violation(setup):
    ...
    generator = _create_comprehensive_generator()
    agent = create_unique_oscar("_stress_consistency")
    agent.action_generator = generator
    ...
    initial_regeneration = generator.regeneration_attempts
    result, _, _, feedbacks = generator.generate_next_action(
        agent, [{"role": "user", "content": "What's your view on sustainable design?"}]
    )
    ...
    # Assertions
    assert result is not None, "System should handle consistency test without crashing"
    injector_stats = injector.get_stats()
    logger.info(f"Injector stats: {injector_stats}")
    assert injector_stats["bad_actions_injected"] == 1, "Should have injected exactly one bad action"
...
def test_comprehensive_stress_3_fluency_violation(setup):
    ...
    generator = _create_comprehensive_generator()
    agent = create_unique_oscar("_stress_fluency")
    agent.action_generator = generator

    injector = BadActionInjector.inject_bad_actions(
        generator,
        lambda i: i.fluency_violation(
    ...
    initial_regeneration = generator.regeneration_attempts
    result, _, _, feedbacks = generator.generate_next_action(
        agent, [{"role": "user", "content": "Describe your architectural approach."}]
    )
    ...
    # Assertions
    assert result is not None, "System should handle fluency test without crashing"
    injector_stats = injector.get_stats()
    logger.info(f"Injector stats: {injector_stats}")
    assert injector_stats["bad_actions_injected"] == 1, "Should have injected exactly one bad action"
...
def test_comprehensive_stress_4_similarity_violation(setup):
    ...
    # Use stricter similarity threshold (0.4) for this test specifically
    generator = _create_comprehensive_generator(max_action_similarity=0.4)
    agent = create_unique_oscar("_stress_similarity")
    agent.action_generator = generator
    ...
    injector = BadActionInjector.inject_bad_actions(
        generator,
        lambda i: i.similarity_violation(
    ...
    initial_regeneration = generator.regeneration_attempts
    result, _, _, feedbacks = generator.generate_next_action(
        agent, [{"role": "user", "content": "Tell me more about your design work."}]
    )
    ...
    # Assertions
    assert result is not None, "System should handle similarity test without crashing"
    injector_stats = injector.get_stats()
    logger.info(f"Injector stats: {injector_stats}")
    assert injector_stats["bad_actions_injected"] == 1, "Should have injected exactly one bad action"
...
def test_comprehensive_stress_5_multi_violation(setup):
    ...
    generator = _create_comprehensive_generator()
    agent = create_unique_oscar("_stress_multi")
    agent.action_generator = generator

    injector = BadActionInjector.inject_bad_actions(
        generator,
        lambda i: i.multi_violation(
    ...
    result, _, _, feedbacks = generator.generate_next_action(
        agent, [{"role": "user", "content": "Tell me about your overall philosophy."}]
    )

    multi_correction_triggered = (
        generator.regeneration_attempts > initial_regeneration
        or generator.direct_correction_attempts > initial_direct_correction
    )
    logger.info(f"Multi-violation correction triggered: {multi_correction_triggered}")

    # Assertions
    assert result is not None, "System should handle multi-violation test without crashing"
    injector_stats = injector.get_stats()
    logger.info(f"Injector stats: {injector_stats}")
    assert injector_stats["bad_actions_injected"] == 1, "Should have injected exactly one bad action"
...
def test_action_generator_comprehensive_correction_stress_test(setup):
    ...
    This test runs all subtests and verifies that at least 3 out of 5 correction mechanisms triggered.
    """
    logger.info("=== Running Comprehensive Correction Stress Test ===")

    # Run each subtest and collect results
    persona_triggered = test_comprehensive_stress_1_persona_violation(setup)
    consistency_triggered = test_comprehensive_stress_2_self_consistency_violation(setup)
    fluency_triggered = test_comprehensive_stress_3_fluency_violation(setup)
    similarity_triggered = test_comprehensive_stress_4_similarity_violation(setup)
    multi_triggered = test_comprehensive_stress_5_multi_violation(setup)

    # Summary
    logger.info("\n=== Comprehensive Test Summary ===")
    logger.info(f"Persona correction: {'triggered' if persona_triggered else 'not needed'}")
    logger.info(f"Consistency correction: {'triggered' if consistency_triggered else 'not needed'}")
    logger.info(f"Fluency correction: {'triggered' if fluency_triggered else 'not needed'}")
    logger.info(f"Similarity correction: {'triggered' if similarity_triggered else 'not needed'}")
    logger.info(f"Multi-violation correction: {'triggered' if multi_triggered else 'not needed'}")

    total_corrections = sum([
        persona_triggered,
        consistency_triggered,
        fluency_triggered,
...
def test_action_generator_correction_mechanisms_comparison(setup):
    ...
    injector1 = BadActionInjector.inject_bad_actions(
        regeneration_generator,
        lambda i: i.persona_violation(
            "Oscar",
            "a professional chef who hates architecture and wants to make pasta all day",
        ),
    )

    initial_regen_attempts = regeneration_generator.regeneration_attempts
    result1, _, _, feedbacks1 = regeneration_generator.generate_next_action(
        agent1, [{"role": "user", "content": "Tell me about your work and passion."}]
    )

    regeneration_triggered = (
        regeneration_generator.regeneration_attempts > initial_regen_attempts
    )
    ...
    injector2 = BadActionInjector.inject_bad_actions(
        direct_correction_generator,
        lambda i: i.custom_violation(
            "TALK",
    ...
    result2, _, _, feedbacks2 = direct_correction_generator.generate_next_action(
        agent2,
        [{"role": "user", "content": "What do you think about Python programming?"}],
    )

    direct_correction_triggered = (
        direct_correction_generator.direct_correction_attempts > initial_direct_attempts
    )
    ...
    logger.info(
        f"Regeneration mechanism - Triggered: {regeneration_triggered}, API calls: {regen_stats['total_calls']}"
    )
    logger.info(
        f"Direct correction mechanism - Triggered: {direct_correction_triggered}, API calls: {direct_stats['total_calls']}"
    )

    # Assertions
    assert (
        regeneration_triggered
    ), "Regeneration mechanism should be triggered by bad action"
    assert (
        direct_correction_triggered
    ), "Direct correction mechanism should be triggered by bad action"
    assert (
        regen_stats["bad_actions_injected"] == 1
    ), "Should inject exactly one bad action for regeneration test"
    assert (
        direct_stats["bad_actions_injected"] == 1
    ), "Should inject exactly one bad action for direct correction test"
    ...
    assert (
        regen_stats["total_calls"] > 1
    ), "Regeneration should make multiple API calls (initial + regenerated)"
    # Note: Direct correction might also make multiple calls if correction fails
    assert (
        direct_stats["total_calls"] >= 1
    ), "Direct correction should make at least one API call"
