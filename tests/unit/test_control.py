...
def test_begin_checkpoint_end_with_agent_only(setup):
    ...
    assert (
        control._current_simulations["default"] is None
    ), "There should be no simulation running at this point."
    ...
    assert (
        control._current_simulations["default"].status == Simulation.STATUS_STARTED
    ), "The simulation should be started at this point."

    exporter = ArtifactExporter(base_output_folder="./synthetic_data_exports_3/")
    enricher = TinyEnricher()
    tooluse_faculty = TinyToolUse(
    ...
    assert (
        control._current_simulations["default"].cached_trace is not None
    ), "There should be a cached trace at this point."
    assert (
        control._current_simulations["default"].execution_trace is not None
    ), "There should be an execution trace at this point."
    ...
    assert os.path.exists(
        "control_test.cache.json"
    ), "The checkpoint file should have been created."

    control.end()

    assert (
        control._current_simulations["default"].status == Simulation.STATUS_STOPPED
    ), "The simulation should be ended at this point."
...
def test_begin_checkpoint_end_with_world(setup):
    ...
    world = TinyWorld(
        "Test World", [create_oscar_the_architect(), create_lisa_the_data_scientist()]
    )
...
def test_begin_checkpoint_end_with_factory(setup):
    ...
    def aux_simulation_to_repeat(iteration, verbose=False):
        ...
        factory = TinyPersonFactory(
            "We are interested in experts in the production of the traditional Gazpacho soup."
        )
        ...
        agent = factory.generate_person(
            "A Brazilian tourist who learned about Gazpaccho in a trip to Spain."
        )
        ...
        # check if the file was created
        assert os.path.exists(
            "control_test_personfactory.cache.json"
        ), "The checkpoint file should have been created."
        ...
        if verbose:
            logger.debug(
                f"###################################################################################### Sim Iteration:{iteration}"
            )
            logger.debug(
                f"###################################################################################### Agent persona configs:{agent._persona}"
            )
    ...
    ), "The nationality should be the same in both simulations."
    ...
    assert (
        "'_aux_model_call'" in cache_contents
    ), "The cache file should contain the '_aux_model_call' call."
    assert (
        "'_setup_agent'" in cache_contents
    ), "The cache file should contain the '_setup_agent' call."
    assert (
        "'define'" not in cache_contents
    ), "The cache file should not contain the 'define' methods, as these are reentrant."
    assert (
        "'define_several'" not in cache_contents
    ), "The cache file should not contain the 'define_several' methods, as these are reentrant."
