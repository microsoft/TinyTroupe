...
class TestExtendedScenarios:
    ...
    def test_story_telling_scenario(self, setup):
        ...
        writer = factory.generate_person(
            "A creative writer who loves storytelling, and always insists in including dogs in all stories."
        )
        editor = factory.generate_person(
            "An editor with strong narrative sense, and always insists in including cats in all stories."
        )
        ...
        story_content = extractor.extract_results_from_world(
            world,
        ...
        assert proposition_holds(
            f"The following contains a narrative story with characters and plot: '{story_content}'"
        ), "Story should contain narrative elements"
        assert proposition_holds(
    ...
    def test_synthetic_conversation_generation(self, setup):
        ...
        world.broadcast(
            "You need to discuss the upcoming software project deadline. Share your perspectives and work together to find solutions."
        )
        world.run(3)
        ...
        assert len(conversations) > 0, (
            f"Should have generated conversations. Found {len(conversations)} conversations."
        )
    ...
    def test_product_market_research_scenario(self, setup):
        ...
        for i in range(3):
            consumer = factory.generate_person(
                f"A consumer with distinct preferences and background, person {i+1}."
            )
        ...
        for consumer in consumers:
            ...
            response = extractor.extract_results_from_agent(
                consumer,
        ...
        for response in responses:
            assert response["decision"] in ["Yes", "No", "Maybe"], (
                f"Decision should be Yes/No/Maybe, got: {response['decision']}"
            )
            assert len(response["reasoning"]) > 10, "Should provide reasoning for decision"

    def test_political_opinion_polling_scenario(self, setup):
        """
        ...
        conservative = factory.generate_person(
            "A conservative-leaning voter concerned about traditional values and fiscal responsibility."
        )
        liberal = factory.generate_person(
            "A liberal-leaning voter focused on social justice and environmental issues."
        )
        moderate = factory.generate_person(
            "A moderate voter who considers multiple perspectives on issues."
        )
        ...
        for agent in agents:
            ...
            opinion = extractor.extract_results_from_agent(
                agent,
    ...
    def test_multi_agent_problem_solving_scenario(self, setup):
        ...
        analyst.listen_and_act(
            "Please summarize the key solutions our team has identified for rural healthcare delivery."
        )

        # Extract problem-solving results
        extractor = ResultsExtractor()
        solution = extractor.extract_results_from_agent(
            analyst,
        ...
        assert proposition_holds(
            f"The following contains practical solutions for rural healthcare delivery: '{solution}'"
        ), "Should contain healthcare solutions"
        assert proposition_holds(
            f"The following shows evidence of multi-disciplinary collaboration: '{solution}'"
        ), "Should show collaborative problem-solving"
